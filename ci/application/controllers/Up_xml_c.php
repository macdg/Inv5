<?php
class Up_xml_c extends CI_Controller {
...
public function __construct() {
	...
	$this->load->database();
	$this->load->model('Up_xml_model_c', 'up_xml_model_c');
...
public function subiendo_archivo() {
	...
	if (is_uploaded_file($_FILES['mi_archivo_1']['tmp_name'])) {
	 	...
	 	if (file_exists($nombreDirectorio.$nombreFichero)) {
			...
			$val_p=$this->load->view('success_c', $datos);
			return ($val_p);
			exit();
		}
				...
				$exten2=".XML";
					...
					if ( ($val_ext != $exten1) && ($val_ext != $exten2) ) {
						...
						$val_p1=$this->load->view('success_c', $datos1);
 			...
 			$datos2['no_up'] = TRUE;
			$val_up=$this->load->view('success_c', $datos2);
			return ($val_up);
			exit();
		}
						...
						foreach ($result['cfdi:Comprobante']['attr'] as $cy => $ll) {
							...
							foreach ($moneda1 as $value => $key) {
								...
								foreach ($moneda2 as $value_m => $key_m) {
									...
									$this->load->view('form_xml_c',$datos_concepto);
...
public function x($datos_concepto, $datos_proveedor, $datos_factura, $cont) {
	...
	$q_id_uuid=$this->up_xml_model_c->query_iduuid();
		foreach ($q_id_uuid as $key => $value_iduuid) {
			if ($value_iduuid === $igual) {
				$datos3['m3'] = TRUE;
				$si_existe=$this->load->view('success_c', $datos3);
				return ($si_existe);
				exit();
			}
	 	}

			$this->up_xml_model_c->insert_concepto($datos_concepto, $cont);
	 	 	foreach ($q_id_uuid as $valu){
				...
				$this->up_xml_model_c->factura_data_insert(
				$id_uuid,
				$fecha,
				$subtotal,
 				$moneda,
				...
				$q_idrfc_proveedor=$this->up_xml_model_c->query_idrfc();
				foreach ($q_idrfc_proveedor as $row)
					...
					if ($cambiar != $id_rfc ) {
						...
						$this->up_xml_model_c->proveedor_data_insert(
						$id_rfc,
						$rfc_nom,
						$calle,
						$no_ext,
	...
	foreach ($noserie as $key => $value)
		$varx=$value;
			...
			if ($varx == NULL) {
					...
					$this->form_validation->set_rules('noserie', 'Número de Serie', 'required');
					 echo validation_errors();
						...
						if ($this->form_validation->run()==FALSE) {
							$datos4['m4'] = TRUE;
							$valida_camp=$this->load->view('success_c', $datos4);
							return ($valida_camp);
							exit();

							$q_idproductos=$this->up_xml_model_c->query_idproductos();
				...
				if ($varx != NULL ) {
					$this->up_xml_model_c->insert_in($noserie, $elementos);
				...
				// ***Bloque para traer el id_concepto de la tabla CONCEPTO hacia el registro id_concepto de la tabla PRODUCTOS de acuerdo a la cantidad de registros ingresados por el usuario. ****
					$q_id_c_c_concepto=$this->up_xml_model_c->id_c_c_concepto();
					...
					$q_id_c_p_productos=$this->up_xml_model_c->id_c_p_productos();
				...
				// *** Bloque independiente ****
					for ($i=0; $i<$contando_split; $i++) { 
						...
						$this->up_xml_model_c->update_productos($d_ant_productos, $n_id_concepto, $dato_de_id_productos);
				// *** Fin de Bloque. ***

						$consulta_vacio=$this->up_xml_model_c->con_noserie_productos();
						foreach ($consulta_vacio->result_array() as $rw) {
					   		$evaluar=$rw['noserie'];

							while ($evaluar == NULL) {
								$consulta2 = $this->up_xml_model_c->id_productos_productos();
								$d_ant2=$consulta2;
								$datos4['m4'] = TRUE;
								$valida_camp=$this->load->view('success_c', $datos4);
								return ($valida_camp);
					...
					$finalizar=$this->load->view('success_c', $d_final);
					return ($finalizar);
					exit();
	...
	for ($i=0; $i<$cont_ant; $i++) { 
		...
		foreach ($quitar as $keyq => $valueq)
			...
			if (isset($quitar)) {
				...
				$q_res=$this->up_xml_model_c->q_del_elemento();
						...
						for ($u=0; $u<$cont_ant; $u++)
							...
							foreach ($qt as $keyq => $valq)
								...
								foreach ($id_new as $keyn => $valuen) {
									...
									$this->up_xml_model_c->del_elem($c);
